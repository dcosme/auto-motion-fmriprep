...
#     theme_minimal()
# ggsave(p, file = 'plots/correlations.png', height = 30, width = 30)
```
...
## apply models to holdout sample
```{r}
...
plot.train = joined %>%
  mutate(artifact = as.factor(ifelse(artifact == 1, "yes", "no")))
plot.train$predicted = predict(rf_diffs, ml.data)
...
plot.compare = joined %>%
  mutate(artifact = as.factor(ifelse(artifact == 1, "yes", "no")))
plot.compare$predicted.ML = predict(orig_rf_diffs, ml.data)
...
confusionMatrix(mat$artifact, as.factor(mat$predicted.2mm))
confusionMatrix(mat$artifact, as.factor(mat$predicted.fd))
confusionMatrix(mat$artifact, as.factor(mat$predicted.ML))
```
...
rps[is.na(rps)] = 0
rps$ML = predict(orig_rf_diffs, rps)
